...
static int sepol2ipproto(sepol_handle_t * handle, int proto)
{
...
static int ipproto2sepol(sepol_handle_t * handle, int proto)
{
...
extern int sepol_port_count(sepol_handle_t * handle, const sepol_policydb_t * p, unsigned int *response)
{
		      ...
		      const sepol_port_key_t * key, int *response)
{
	...
	*response = 0;
	return STATUS_SUCCESS;
      ...
      err:
	ERR(handle, "could not check if port range %u - %u (%s) exists", low, high, sepol_port_get_proto_str(proto));
	return STATUS_ERR;
}
		     ...
		     const sepol_port_key_t * key, sepol_port_t ** response)
{
	...
	*response = NULL;
	return STATUS_SUCCESS;
      ...
      err:
	ERR(handle, "could not query port range %u - %u (%s)", low, high, sepol_port_get_proto_str(proto));
	return STATUS_ERR;
		      ...
		      const sepol_port_key_t * key, const sepol_port_t * data)
{
	...
	return STATUS_SUCCESS;
      ...
      err:
	ERR(handle, "could not load port range %u - %u (%s)", low, high, sepol_port_get_proto_str(proto));
	if (port != NULL) {
