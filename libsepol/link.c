...
#ifdef DEBUG
			const char *mod_name = cur->branch_list->module_name ? cur->branch_list->module_name : "BASE";
#endif
				       ...
				       missing_requirement_t * req)
{
	...
	if (req->symbol_type == SYM_CLASSES) {
		...
		ERR(state->handle,
		    ...
		    cur->branch_list->module_name ? cur->branch_list->module_name : "BASE"
		...
		for (block = pol->global; block != NULL; block = block->next) {
			...
			if (state->verbose) {
				INFO(state->handle, "check module %s decl %d\n",
				     decl->module_name ? decl->module_name : "BASE",
					 decl->decl_id);
			}
