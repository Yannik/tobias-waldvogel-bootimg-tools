...
static int mls_level_cpy(struct mls_level *dst, struct mls_level *src)
{
...
static void mls_level_init(struct mls_level *level)
{

	memset(level, 0, sizeof(mls_level_t));
}

static void mls_level_destroy(struct mls_level *level)
{
...
static int mls_level_eq(const struct mls_level *l1, const struct mls_level *l2)
{
	return ((l1->sens == l2->sens) && ebitmap_cmp(&l1->cat, &l2->cat));
}

static int mls_level_dom(const struct mls_level *l1, const struct mls_level *l2)
{
...
static int mls_range_cpy(mls_range_t * dst, mls_range_t * src)
{
...
static void mls_range_init(struct mls_range *r)
{
	mls_level_init(&r->level[0]);
	mls_level_init(&r->level[1]);
}

static void mls_range_destroy(struct mls_range *r)
{
	mls_level_destroy(&r->level[0]);
	mls_level_destroy(&r->level[1]);
}

static int mls_range_eq(struct mls_range *r1, struct mls_range *r2)
{
